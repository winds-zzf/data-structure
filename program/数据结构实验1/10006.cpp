...
//根据图书名称来查找该本书
void  SearchbyName(SqList L, Book e)
{
	int counter = 0;
	for (int i = 0; i < L.length; i++)
	{
		if (!strcmp(L.base[i].bookName, e.bookName)) counter++;
	}
	...
	else
	{
		...
		for (int i = 0; i < L.length; i++)
		{
			if (!strcmp(L.base[i].bookName, e.bookName)) Output(L.base[i]);
		}
	}
}
...
int main()
{
	...
	{
		scanf("%s", e.bookName, 40);		//读取喜爱的书名
		SearchbyName(L, e);					//进行相应的查找
	}
	return 0;
}