...
//将book书单按照价格排序
void EditPrice(SqList L)
{
	float sum = 0, aver;
	for (int i = 1; i <= L.length; i++)
	{
		sum += L.base[i-1].bookPrice;
	}
	aver = sum / L.length;
	printf("%.2f\n", aver);
	for (int i = 1; i <= L.length; i++)
	{
		if (L.base[i - 1].bookPrice < aver)
			L.base[i - 1].bookPrice *= 1.2;
		else L.base[i - 1].bookPrice *= 1.1;
	}
}
int main()
{
