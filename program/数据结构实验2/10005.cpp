...
int main()
{
	Book e, max = { "0", "0", 0.0 };		//book类型变量存储书籍信息
	SqList L;
	int counter;	
	InitList(L);
	...
	{
		Input(e); //读入信息
		L.base[i] = e;		//直接修改指定位置的值
		if (e.bookPrice > max.bookPrice)
		{
			max = e;	//在输入是查找最大值
			counter=1;	//当出现新的最大值是，计数器重新计数
		}
		else if(e.bookPrice == max.bookPrice)
		{
			counter++;	//若当前记录最大值再次出现，计数器加一
		}
	}
	printf("%d\n", counter);
	for (int i = 0; i < L.length; i++)
	{
		if (L.base[i].bookPrice == max.bookPrice)	Output(L.base[i]);
	}
	return 0;
}